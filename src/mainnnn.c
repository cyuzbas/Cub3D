...
/*   Updated: 2023/03/08 17:48:26 by cyuzbas       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void hook(void* param)
{
	...
	{
		player->instances[0].angle -= 0.05 *  M_PI;
		if (player->instances[0].angle < 0)
			player->instances[0].angle += 2 * M_PI;
	...
	{
		player->instances[0].angle += 0.05 *  M_PI;
		if (player->instances[0].angle > 2 * M_PI)
			player->instances[0].angle -= 2 * M_PI;
		player->instances[0].dx = cos(player->instances[0].angle * 5.0);
		player->instances[0].dy = sin(player->instances[0].angle * 5.0);
	}
...
void draw_direction(mlx_image_t* player, t_map map, int x, int y)
{
    ...
    for(int i = 0; i < 10; i++)
    {
		// player->instances[0].angle = 1;
		// printf("angle %f", player->instances[0].angle);
        x1 = (i * cos((1.5 * M_PI))) + x;
        y1 = (i * sin((1.5 * M_PI))) + y;
		// printf("x %d X1 %f y %d  y1 %f i %d cos %d sin %d\n",x,x1,y,y1,i, (int)cos(2 * M_PI) , (int)sin(2 * M_PI));
		// printf("x %d y %d ,angel value = %f, y value = %f\n",data->player.x, data->player.y ,data->player.d_x, data->player.d_y);	
...
void draw_player(mlx_t* mlx, mlx_image_t* player, t_map map)
{
	int size = 6;
	int x;
	int y = (map.y*map.ratio)+(map.ratio-size)/2;

	while (y < (map.y*map.ratio)+(map.ratio-size)/2 + size)
	{
		x = (map.x*map.ratio)+(map.ratio-size)/2;
		while (x < (map.x*map.ratio)+(map.ratio-size)/2 + size)
		{
	...
	draw_direction(player, map, y-size/2, x-size-1);
...
int	main(void)
{
	...
	player = mlx_new_image(mlx,  map.len * map.ratio, map.line * map.ratio); //create two images in the size of window
